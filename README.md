...
- **50+ server-driven event adapters** — `OnClickAdapter`, `OnChangeAdapter`, `OnSubmitAdapter`, `OnDragAdapter`, `OnKeyDownAdapter`, `OnMouseEnterAdapter`, and many more — each backed by a `ServiceLoader` SPI (`IOnClickService`, `IOnChangeService`, etc.)
...
- **Feature system** — `Feature<O, J>` wraps a JavaScript library with typed options (`JavaScriptPart`), CSS/JS references, and automatic dependency ordering
