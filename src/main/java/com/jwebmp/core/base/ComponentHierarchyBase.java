...
public class ComponentHierarchyBase<C extends GlobalChildren,
    ...
    public J setTiny(boolean tiny) {
        getChildren().stream().filter(a -> a != null).forEach(child -> child.asBase().setTiny(tiny));
    ...
    public J setPage(Page<?> page) {
        this.page = page;
        getChildren().stream().filter(a->a!=null).forEach(child -> child.asHierarchyBase().setPage(page));
        return (J) this;
    }
