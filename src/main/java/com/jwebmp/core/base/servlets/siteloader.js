    ...
    if(jw.websocket !== undefined && jw.websocket && jw.websocket.connected) {
        ...
        $.ajax({
            ...
            success: function (result, status, xhr) {
                try {
                    jw.actions.processResponse(result, jw.env.scope, jw.env.parse, jw.env.timeout, jw.env.compile, jw.env.rootScope);
                } catch (e) {
 ...
 */
jw.getEventObject = function ($event,compID) {
    var newEvent = {};
...
jw.makeEmptyArticle = function(compID,className,dataVariables){
    var article = {};
    ...
    article.value = jw.getEventObject(undefined,compID);
...
jw.perform = function ($event, dataVariables, eventId, className) {
    jw.isLoading = true;
    if (window.Pace) {
        window.Pace.options.maxProgressPerFrame = 0.2;
        window.Pace.stop();
