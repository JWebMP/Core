		...
		extends JWDefaultServlet
{
	 ...
	 */
	private static final Logger SESSION_LOG = LogFactory.getInstance()
	                                                    .getLogger("SessionHandler");
	...
	private void readRequestVariables(HttpServletRequest request) throws MissingComponentException
	{
		...
		if (session.isNew())
		{
			SESSION_LOG.log(Level.FINER, "[SessionID]-[{0}];[Name]-[User Login];[Action]-[Session Page Added];", request.getSession()
			                                                                                                            .getId());
		}
	}
	...
	private void sendPage(HttpServletRequest request, HttpServletResponse response) throws IOException
	{
		...
		try (PrintWriter out = response.getWriter())
		{
			...
			log.log(Level.FINE, "[SessionID]-[{0}];[Render Time]-[{1}];[Data Size]-[{2}];[Transer Time]=[{3}]",
			        new Object[]{request.getSession().getId(), endDate.getTime() - startDate.getTime(), output.length(), dataTransferDate.getTime() - transferStart.getTime()});
		}
		catch (IOException ex)
		{
